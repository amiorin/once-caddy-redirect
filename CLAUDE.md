...
A minimal containerized HTTP redirector: every request on port 80 is 302-redirected to `https://www.bigconfig.ai{uri}`, with a `/up` health endpoint returning `200 OK`. There is no application code — the entire behavior lives in `Caddyfile`.

## Architecture

- **`Caddyfile`** — Caddy config. `auto_https off` and `protocols h1` because the container only serves plain HTTP/1.1 on `:80` (TLS is expected to terminate upstream, e.g. at a load balancer/CDN). Two handlers: `/up` for health checks, catch-all `handle` for the temporary redirect preserving path + query via `{uri}`.
...
curl -I http://localhost:8080/        # expect 302 to www.bigconfig.ai/
